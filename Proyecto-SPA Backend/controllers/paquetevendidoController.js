...
const PaqueteVendidoController = {
  ...
  getByClienteActivos: async (req, res, next) => {
    try {
  ...
  getByClienteExpirados: async (req, res, next) => {
    try {
  ...
  getById: async (req, res, next) => {
    try {
      const data = await PaqueteVendidoService.getById(req.params.id,req.params.id2,req.params.id3);
      if (!data) {
  ...
  delete: async (req, res, next) => {
    try {
      console.log('paso por cotroller')
